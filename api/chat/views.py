...
from chat.schemes import ChatCreate, ChatRemove
...
@login_required
async def remove_chat(data: ChatRemove, user: User = Depends(get_current_active_user)):
    fetch = ChatTable.select().where(ChatTable.c.id == data.id)
    participants = await get_chat_participants(data.id)
    receivers = [p.id for p in participants]
    async with db.connect() as conn:
        res = await conn.execute(fetch)
        chat = res.fetchone()
        if chat['type'] == ChatType.PRIVATE.value or (
                chat['type'] == ChatType.GROUP.value and chat['creator'] == user.id):
            query = ChatTable.delete().where(ChatTable.c.id == data.id)
            remove_data = json.dumps(
                {'id': data.id, 'receivers': receivers, 'action': ChatAction.REMOVE.value}
            )
            await ws_user_manager.notify_subscribers(remove_data)
        else:
            query = ParticipantTable.delete() \
                .where(ParticipantTable.c.data.id == data.id, ParticipantTable.c.user_id == user.id)
            leave_data = json.dumps(
                {'id': data.id, 'receivers': receivers, 'action': ChatAction.LEAVE.value}
            )
